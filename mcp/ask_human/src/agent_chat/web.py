...
"""Web dashboard for answering ask_human questions (stdlib only).
    ...
    agent-chat web                                  # via the operator console
    python -m agent_chat.web                         # or run the module directly
    ASK_HUMAN_WEB_ADDR=0.0.0.0:9000 agent-chat web   # bind elsewhere
...
function renderDetail(){
  ...
  if(q.options == null) actions.appendChild(h("span", "hint", "⌘/Ctrl + Enter"));
...
async function submit(ev){
  ...
  if(q.options){
    ...
    if(!checked.length){ toast("Pick an option first."); return; }
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: N802
        ...
        if question and question["status"] == "pending":
            answer = answers if question["multi_select"] else (answers[0] if answers else "")
            ok = store.answer(qid, answer, answered_by="web")
