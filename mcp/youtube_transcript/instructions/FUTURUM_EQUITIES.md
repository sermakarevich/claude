...
2. Each agent calls `mcp__youtube_transcript__get_transcript(video=<bare-id>)` and extracts:
   - tickers to BUY + reasoning
   - tickers to SELL + reasoning
   - tickers OWNED in video + reasoning
3. Aggregate findings → save as `mcp/youtube_transcript/outputs/FUTURUM_EQUITIES_{DATE}.md` (DATE = YYYY-MM-DD) with cross-video tally (B=Buy, S=Sell, O=Owned mention counts) + per-video sections.
