...
from fleet.attempts import increment_failure
from fleet.config import RuntimeConfig, load, reload_if_changed
            ...
            case TaskOutcome.FAILURE:
                ...
                if new_count >= self.config.retry_limit:
                    ...
                    self._queue.comment(
                        task.id,
                        (
                            f"[fleet] retry limit exhausted after {new_count} attempts. "
                    ...
                    self._queue.comment(
                        task.id,
                        (
                            f"[fleet] attempt {new_count} failed (rc={outcome.exit_code}). "
