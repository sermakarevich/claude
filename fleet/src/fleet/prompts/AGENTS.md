...
> **Note:** This file is the `AGENTS.md` equivalent of `CLAUDE.md`, intended
> for adapters that read `AGENTS.md` (e.g. Codex, or future non-Claude
> adapters). The content is identical to `CLAUDE.md` — consult that file for
> the authoritative version. When adding a new adapter, ship one class file +
> one prompt file (this file or CLAUDE.md) and register the adapter.
