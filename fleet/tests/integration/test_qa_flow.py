...
def test_qa_block_and_resume(tmp_path: Path) -> None:
    ...
    # Attempt 1: block_via_bd — agent writes Q block and marks blocked via bd
    # Attempt 2: read_qa_and_close — agent reads A block and closes via bd
    ...
    qa_path = artifact_root / task_id / "Q&A.md"
    ...
    sup2 = make_supervisor(tmp_path, queue, adapter=adapter, config=config)
    ...
    # events.jsonl should have records from both attempts
    import json
    events_path = artifact_root / task_id / "events.jsonl"
...
def test_qa_blocked_no_failure_count(tmp_path: Path) -> None:
    ...
    assert done.is_set()
    artifact_dir = tmp_path / ".claude" / "tasks" / task_id
