...
def test_dynamic_config_max_concurrent_reloads(tmp_path: Path) -> None:
    ...
    adapter = FakeClaudeAdapter(scenario="slow", FAKE_CLAUDE_SLEEP_SEC="10")
    config = fast_config(
        max_concurrent=4,
...
def test_day_night_reserve_resume(tmp_path: Path) -> None:
    ...
    adapter = FakeClaudeAdapter(scenario="clean_exit")
    config = fast_config(
        max_concurrent=3,
