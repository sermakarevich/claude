"""Tests for ClaudeCLIAdapter.write_runtime_config (test-after)."""
import json
import stat
...
class TestFirstCallWritesSettings:
    def test_settings_json_matches_fixture(self, adapter, project):
        adapter.write_runtime_config(project, object())
    ...
    def test_hook_scripts_byte_equal_to_shipped(self, adapter, project):
        adapter.write_runtime_config(project, object())
        shipped_dir = ClaudeCLIAdapter._shipped_hooks_dir()
        for name in ("precompact.sh", "pretool_askuserquestion.sh"):
...
class TestMergeSafety:
    def test_user_owned_hook_entries_survive(self, adapter, project):
    ...
    def test_fleet_entry_added_alongside_user_entry(self, adapter, project):
        claude_dir = project / ".claude"
        claude_dir.mkdir(parents=True)
    ...
    def test_unrelated_top_level_settings_preserved(self, adapter, project):
        claude_dir = project / ".claude"
        claude_dir.mkdir(parents=True)
...
class TestIdempotency:
    def test_second_call_produces_byte_identical_settings(self, adapter, project):
        adapter.write_runtime_config(project, object())
    ...
    def test_second_call_produces_byte_identical_hook_scripts(self, adapter, project):
        adapter.write_runtime_config(project, object())
        first = {
    ...
    def test_fleet_managed_entries_not_duplicated(self, adapter, project):
        adapter.write_runtime_config(project, object())
        adapter.write_runtime_config(project, object())
